...
const { fetchInvoice } = require('./invoice')
...
module.exports = async bp => {
  ...
  bp.checkInvoice = (bank, channel) => {
    const channels = await bp.slack.getChannels()
    const { id } = channels.find(({ name }) => name === channel)
    const lastInvoiceRequestDate = bp.kvs.get('lastInvoiceRequestDate')
    const start = lastInvoiceRequestDate || new Date()
    const end = new Date()
    const invoices = await fetchInvoice(start, end, bank)
  
    if (invoices.length) {
      invoices.forEach(({ name, total, date } = {}) => {
        if (!name) {
          return
        }

        bp.slack.sendText(id, `${name} - ${total / 100} - ${date}`)
      })
    }

    bp.kvs.set('lastInvoiceRequestDate', new Date().toJSON())
  }
}
